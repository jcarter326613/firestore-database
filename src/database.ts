...
export class ReservedDocumentPropertyError extends Error {
    constructor() {
        super(
            `"${DOCUMENT_VERSION}" is managed by the Firestore database facade.`,
        )
...
type DocumentFor<Definition extends CollectionDefinition> =
    z.output<Definition["schema"]> extends object
...
export type DatabaseCollections<Definitions extends CollectionDefinitions> = {
    [Name in keyof Definitions]: DatabaseCollection<
        DocumentFor<Definitions[Name]>
    >
}
...
export type DatabaseDocumentOperation<
    Definitions extends CollectionDefinitions,
> = {
...
> {
    forEachDocument<Name extends keyof Definitions & string>(options: {
        ...
        change: (
            ...
            context: {
                newId<Collection extends keyof Definitions & string>(
...
export function defineDatabaseMigrations<
    Definitions extends CollectionDefinitions,
>(
...
const databases = new Map<
    string,
    {
        ...
        database: FirestoreDatabase<CollectionDefinitions>
    }
>()
...
function collectionFacade<T extends object>(
    ...
    const withTransaction = <Result>(
        ...
        transaction === undefined
            ? firestore.runTransaction(operation)
    ...
    return {
        async create(data) {
            ...
            await withTransaction(async (activeTransaction) => {
                activeTransaction.create(
                    collection.doc(id),
                    stored(validated, currentVersion),
                )
            })
...
>(
    ...
    const configurationKey = JSON.stringify({
        collections: Object.entries(options.collections).map(
    ...
    const collectionsFor = (
        transaction?: Transaction,
    ): DatabaseCollections<Definitions> =>
        Object.fromEntries(
            ...
            async run(context) {
                await migration.run({
                    async forEachDocument(processing) {
                        const definition =
                            options.collections[processing.collection]
                        if (!definition) {
                            throw new Error(
                        ...
                        return context.forEachDocument({
                            ...
                            async change(
                                ...
                                const document = {
                                    ...
                                    DocumentFor<
                                        Definitions[typeof processing.collection]
                                    >
                                >
                                const operations = await processing.change(
                                    document,
                                    {
                                        newId(name) {
                                            const target =
                                                options.collections[name]
                                            if (!target) {
                                                throw new Error(
                                                    `Migration "${migration.id}" refers to an unknown collection "${name}".`,
                                                )
                                            }
                                        ...
                                        read: {
                                            collections: collectionsFor(
                                                transaction,
                                            ) as DatabaseReadCollections<Definitions>,
                                        },
                                    },
                                )
    ...
    databases.set(options.databaseId, {
        configurationKey,
        database:
            database as unknown as FirestoreDatabase<CollectionDefinitions>,
    })
    return database
}
