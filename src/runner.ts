...
function assertLeaseData(data: DocumentData | undefined, lease: Lease): void {
    if (
...
function migrationContext(
    ...
    const assertRunningMigration = (data: DocumentData | undefined) => {
        if (
            data?.status !== "running" ||
            data.checksum !== migration.checksum
        ) {
    ...
    return {
        async forEachDocument(
            ...
            const stepReference = ledgerReference
                .collection("steps")
                .doc(options.name)
            ...
            for (;;) {
                ...
                try {
                    page = await firestore.runTransaction(
                        async (transaction) => {
                            const [
                                leaseSnapshot,
                                ledgerSnapshot,
                            ...
                            for (const document of querySnapshot.docs) {
                                ...
                                const operations = await options.change(
                                    document,
                                    transaction,
                                    firestore,
                                )
                            ...
                            for (const { document, operations } of changes) {
                                ...
                                for (const operation of operations) {
                                    ...
                                    if (operation.type === "delete") {
                                        ...
                                        sourceDeleted ||=
                                            reference.path === document.ref.path
                            ...
                            const previousChanged =
                                continuingStep &&
                                typeof stepData?.changed === "number"
                                    ? stepData.changed
                                    : 0
                            const previousProcessed =
                                continuingStep &&
                                typeof stepData?.processed === "number"
                                    ? stepData.processed
                                    : 0
                            transaction.set(
                                stepReference,
                                {
                                    ...
                                    cursor:
                                        ...
                                        null,
                                    processed:
                                        previousProcessed + querySnapshot.size,
                    ...
                    if (
                        effectivePageSize > 1 &&
                        isTransactionSizeError(error)
                    ) {
                        effectivePageSize = Math.max(
                            1,
                            Math.floor(effectivePageSize / 2),
                        )
                        continue
                    }
                    throw error
    ...
    try {
        ...
        for (const migration of pending) {
            ...
            try {
                ...
                await firestore.runTransaction(async (transaction) => {
                    const leaseSnapshot = await transaction.get(
                        references.lease,
                    )
                    assertLeaseData(leaseSnapshot.data(), lease)
                    transaction.set(
        ...
        await releaseLease(firestore, references.lease, lease).catch(
            () => undefined,
        )
    }
}
