...
if (!process.env.FIRESTORE_EMULATOR_HOST) {
  throw new Error("Firestore integration tests require FIRESTORE_EMULATOR_HOST.");
}
    ...
    await migration.forEachDocument({
      ...
      async change(recipe, { newId, read }) {
        ...
        const notes = await read.collections.notes.query({
          where: [{ field: "recipeId", operator: "==", value: recipe.id }],
        });
        return [
          {
            collection: "recipes",
...
const migrations = defineDatabaseMigrations<typeof collections>([firstMigration]);
...
describe.sequential("Firestore emulator integration", () => {
  ...
  it("processes markerless documents with related reads and generated child IDs", async () => {
    await firestore.collection("integration-recipes").doc("recipe-one").set({
      name: "One",
    });
    await firestore.collection("integration-recipes").doc("recipe-two").set({
      name: "Two",
    });
    ...
    const ingredients = await firestore.collection("integration-ingredients").get();
  ...
  it("rejects a second runner while the first holds the lease", async () => {
    ...
    await expect(
      runMigrations(firestore, leaseMigrations, { leaseDurationMs: 10_000 }),
  ...
  it("resumes a failed document-processing step from its saved cursor", async () => {
    ...
    await expect(runMigrations(firestore, resumeMigrations)).rejects.toThrow(
      "deliberate failure",
    );
    await expect(runMigrations(firestore, resumeMigrations)).resolves.toMatchObject({
      applied: [THIRD_MIGRATION_ID],
    });

    const recipes = await firestore.collection("integration-recipes").get();
    expect(recipes.docs.map((document) => document.data().__migrationVersion)).toEqual([
      THIRD_MIGRATION_ID,
      THIRD_MIGRATION_ID,
    ]);
  });
